...
- **2026-07-22** · ⏱️ **E4 U-Net run #3 — cấu hình ĐÚNG (T4 + Internet ON) NHƯNG TIMEOUT, chưa ra số.** Notebook `hlongg28/qigoa-reality-check-e4-u-net` v4, `@cdf1a5a`. Cell 2 xác nhận `torch 2.10.0+cu128 | cuda: True | GPU: Tesla T4` (đã qua kernel-test thật). BraTS 369 ca, cohort **368**, 5 fold cấp bệnh nhân OK. Cell 5 `run_unet.py --resume`: **`[BUDGET] Het 10.5h -> DUNG EM` → `[E4] DUNG (het budget) sau 630.0 phut`** — hết ngân sách, KHÔNG xong. Log cuối: **`Chua co results/unet/raw.csv`**. Output chỉ có **4 checkpoint: `unet_fold0_seed{0,1,2,3}.pt`** (fold 0, seed 0–3) — tức ~2,6h/train trên T4, mới xong 4/25 (target,fold,seed) và chưa kịp inference→raw.csv. Gói `unet_20260721_2135.tgz`.
  - 🔴 **VẤN ĐỀ PHẠM VI (cần quyết định):** 25 tổ hợp × ~2,6h ≈ **~65h GPU** — vượt xa quota Kaggle GPU (~30h/tuần) và cần ~6 session 10,5h. Family A (ceiling decomposition, đóng góp dương #1) chỉ CẦN **1 Dice U-Net out-of-fold/bệnh nhân**; **5 fold × 1 seed = phủ trọn 368 BN** (~13h ≈ 2 session). Đa-seed chỉ cho dải phương sai (E6, "nice-to-have"), KHÔNG cần cho luận điểm phân rã. ⇒ Đề xuất **thu hẹp `unet.seeds` về `[0]`** trước khi resume. Luận đề chính KHÔNG phụ thuộc U-Net (đã xác nhận n=368) — U-Net là TÙY CHỌN.
- **2026-07-22** · `@6ef6d4e` → vá `@HEAD` · Kaggle `hlongg28/qigoa-reality-check-e4-u-net` v5 (T4) · **E4 U-Net run #4 (seeds=[0]) — TIMEOUT nhưng ĐƯỢC 4/5 FOLD; đồng thời LỘ MỘT BUG THẬT.** Cell 2 `GPU: Tesla T4`, cohort 368. Cell 5 lại `[E4] DUNG (het budget) sau 630.0 phut`. Output có **`unet_fold{0,1,2,3}_seed0.pt`** ⇒ **4/5 fold đã train xong**, chỉ còn fold 4.
  - 🔴 **BUG `output_dir` (đã vá).** `section(cfg,'unet')` = `{**cfg, **sub}` (`scripts/_common.py:125`) ⇒ khối `unet:` **thừa kế `output_dir: results/main`** của top-level, **nuốt mất default `"results/unet"`** ở `scripts/run_unet.py:70`. Hậu quả: raw.csv thật ghi vào **`results/main/raw.csv`**, checkpoint vào `results/main/checkpoints/`; notebook kiểm `results/unet/raw.csv` nên in nhầm **`Chua co results/unet/raw.csv`**; Cell 7 đóng gói `results/unet` (RỖNG) ⇒ **`unet_20260722_1154.tgz` vô dụng**. **Rủi ro nặng hơn đã chặn kịp:** session nào restore dữ liệu E2 rồi chạy U-Net sẽ ghi **schema khác đè lên `results/main/raw.csv` của lưới chính**. Vá: khai báo tường minh `unet.output_dir: results/unet` + Cell 7 gói thêm checkpoints và cứu hộ `results/main/raw.csv`.
  - ✅ **KHÔNG mất dữ liệu:** `qigoa/results/main/raw.csv` + 4 checkpoint đều nằm trong output Kaggle, tải về được. Vì `--resume` dựa trên **dòng raw.csv** (`done_keys`, `run_unet.py:94`) **chứ không dựa trên `.pt`**, chỉ cần đưa các dòng fold0–3 vào `results/unet/raw.csv` là session sau **bỏ qua hẳn 4 fold** và chỉ train fold 4 (~2,6h).
  - ⚠️ Số dòng/nội dung `results/main/raw.csv` **CHƯA kiểm chứng** — chờ tải về local rồi mới xác nhận (IRON RULE 3).

---
## E4 U-Net (Family A / ceiling decomposition) — KẾT QUẢ TẠM THỜI 4/5 fold

- **2026-07-22** · **Nguồn số:** `results/unet/raw.csv` ← output Kaggle run #4 (`results/main/raw.csv`, do bug `output_dir` đã vá) ← `scripts/run_unet.py --config configs/exp_main.yaml --resume @6ef6d4e`, notebook `hlongg28/qigoa-reality-check-e4-u-net` v5, GPU Tesla T4, 630 phút.
- **Kiểm toàn vẹn (đã chạy, không phải suy đoán):** **295 dòng = 295 BN duy nhất**; **0 trùng khoá** `(patient_id,target,fold,seed)`; mỗi BN xuất hiện ở **đúng 1 fold** (`max nunique(fold)/BN = 1` ⇒ out-of-fold sạch, **không leakage**, A3 PASS); `empty_mask=0`; `placeholder=0`; target `wt_flair`, method `2D-UNet`, seed 0. Fold 0/1/2/3 = 74/74/74/73. **CÒN THIẾU fold 4 (73 BN)** ⇒ 295/368.
- 🟢🟢 **PHÂN RÃ TRẦN — SO CẶP TRÊN ĐÚNG 295 BN CHUNG** (không so 295 với 368):
  | | median | mean |
  |---|---|---|
  | 2D-UNet (cùng input: 1 lát FLAIR) | **0,9232** | 0,8862 |
  | `oracle_levelset` (trần chỉ-cường-độ) | 0,8593 | 0,8113 |
  | **Δ (U-Net − oracle)** | **+0,0462** | +0,0749 |
  - U-Net thắng trên **78,6% bệnh nhân**; **Wilcoxon p = 7,21e−26**. Nhất quán qua cả 4 fold (median 0,9185 · 0,9238 · 0,9224 · 0,9289).
  - ⇒ **KẾT LUẬN: thông tin NẰM TRONG PIXEL, nhưng thresholding cường độ KHÔNG rút ra được.** Một U-Net 2D trên **đúng cùng một lát FLAIR** vượt **trần lý thuyết của TOÀN BỘ họ phương pháp chỉ-cường-độ** (oracle chọn ngưỡng hoàn hảo) ~4,6 điểm Dice. Gap là *"không nằm trong CƯỜNG ĐỘ"* (cần cấu trúc không gian), **không phải** *"không nằm trong PIXEL"*.
  - ⚠️ **Diễn giải BỊ CẤM:** KHÔNG rút về *"thresholding chạm trần của chính nó"* — François & Tinarrage (JMIV 2026) đã in trần 0,83±0,18 trên BraTS. Đóng góp của ta là **PHÂN RÃ** gap, không phải phát hiện trần.
  - ⚠️ **NHÃN TẠM THỜI:** 295/368 BN. Phải chạy fold 4 rồi tính lại trên đủ 368 trước khi vào bản thảo (IRON RULE 3). Tín hiệu rất nhất quán nên khó đảo chiều, nhưng **con số cuối phải là con số của 368**.
- **2026-07-22** · `@d2783fd` · Kaggle `hlongg28/qigoa-reality-check-e4-u-net` v6 (T4, 144,1 phút) · **🎉 E4 U-Net XONG TRỌN 5/5 FOLD — n=368.** Restore hoạt động đúng thiết kế: `Restore tu: /kaggle/input/datasets/hlongg28/qigoa-e4-u-net-resume-fold0-3/unet_resume_fold0-3.tgz` → `results/unet/raw.csv: 299 dong` ⇒ `--resume` **bỏ qua hẳn fold 0–3**, chỉ `[E4/U-Net] train fold=4 seed=0`. Vá `unet.output_dir: results/unet` xác nhận có hiệu lực: manifest ghi `out_dir=/kaggle/working/qigoa/results/unet/checkpoints`, artifact rơi đúng `results/unet/{raw,summary}.csv`.
  - Cell 5 **`[E4] XONG TRON (rc=0) sau 144.1 phut`** (kết thúc tự nhiên, KHÔNG chạm ngân sách 10,5h). Cell 6: **`(target,fold,seed) da xong: 5 / 5`** · **`so BN co du bao: 368 (ky vong 368)`** · **`Dice U-Net: median=0.9234 IQR=[0.8666, 0.9497]`** · **`CON THIEU: (HET - E4 DAY DU)`**. Manifest: `n_rows=73` (đúng phần fold 4), `seeds=[0]`, `placeholder=false`, `elapsed_s=8313.99`. Gói `unet_20260722_2025.tgz`.
  - **Median trên đủ 368 = 0,9234** so với **0,9232 trên 295 BN tạm thời** ⇒ fold 4 không đổi kết luận. Phân rã trần vẫn giữ nguyên hướng và độ lớn.
  - ⏳ Còn lại: tải tgz về local → thay `results/unet/raw.csv` bản đủ 368 → tính lại so cặp vs `oracle_levelset` trên 368 → `scripts/run_stats.py` điền `family_a_superiority.csv` → gỡ nhãn tạm thời.

---
## E4 / FAMILY A — PHÂN RÃ TRẦN: DÒNG PROVENANCE (số HẾT nhãn tạm thời kể từ 2026-07-22)

- **Nguồn số:** `results/unet/{raw,summary}.csv` + `run-manifest.json` ← `scripts/run_unet.py --config configs/exp_main.yaml --resume @d2783fd`, 2 session Kaggle GPU Tesla T4 (2026-07-22): session A train fold 0–3 (630 phút, chạm ngân sách), session B restore `unet_resume_fold0-3.tgz` + train fold 4 (`XONG TRON rc=0`, 144,1 phút). Gói cuối `unet_20260722_2025.tgz`.
- **Cổng kiểm toàn vẹn (đã chạy trên bản đủ):** **368 dòng = 368 BN duy nhất**; **0 trùng khoá** `(patient_id,target,fold,seed)`; **mỗi BN xuất hiện ở đúng 1 fold** ⇒ out-of-fold sạch, **không leakage** (prereg A3 PASS); fold 0–4 = 74/74/74/73/73; `empty_mask=0`; `placeholder=0`. **Đối chiếu 295 dòng chung với bản session A: khác nhau 0 dòng** ⇒ restore bảo toàn bit-exact.
- **Thống kê:** `results/stats/family_a_superiority.csv` ← `scripts/run_stats.py --config configs/exp_main.yaml` (360,5 s, bootstrap n=10.000, đơn vị = bệnh nhân).

### Số chốt (n=368, `wt_flair`, so CẶP theo bệnh nhân)

| | median | IQR / mean |
|---|---|---|
| **2D-UNet** (cùng input: 1 lát FLAIR, out-of-fold) | **0,9234** | IQR [0,8666 – 0,9497] · mean 0,8870 |
| **`oracle_levelset`** (trần lớp chỉ-cường-độ) | **0,8532** | mean 0,8097 |
| **Δ (U-Net − oracle)** | **+0,0503** | 95% CI [+0,0413; +0,0646] · mean +0,0773 |

- U-Net vượt trần trên **293/368 BN (79,6%)**; **Wilcoxon p = 2,77×10⁻³²**; **rank-biserial = 0,712** (effect size LỚN). Nhất quán cả 5 fold (median 0,9185 · 0,9238 · 0,9224 · 0,9289 · 0,9249).

### Diễn giải ĐƯỢC PHÉP viết

- 🟢 **Phân rã gap:** ~5 điểm Dice mà toàn bộ họ thresholding chỉ-cường-độ **không thể** với tới — kể cả khi được **oracle chọn ngưỡng hoàn hảo** — lại **rút ra được từ ĐÚNG NHỮNG PIXEL ĐÓ** bởi một mô hình dùng cấu trúc không gian. ⇒ Thất bại của thresholding **KHÔNG phải giới hạn thông tin của đầu vào**, mà là **giới hạn của lớp giả thuyết chỉ-cường-độ**. Gap nằm ở *"không có trong CƯỜNG ĐỘ"*, **không phải** *"không có trong PIXEL"*.
- ⛔ **CẤM:** mọi câu dạng *"we establish the ceiling"* / *"thresholding chạm trần của chính nó"* — François & Tinarrage (JMIV 68:20, 2026) đã in trần 0,83±0,18 trên BraTS FLAIR. Ta **cite trang trọng** và claim **PHÂN RÃ + định lượng**, KHÔNG claim phát hiện trần. (Trường `fallback_if_not_superior` trong CSV là nhánh dự phòng cho trường hợp U-Net **không** vượt — **không áp dụng ở đây** vì U-Net vượt có ý nghĩa thống kê.)
- ⚠️ **Phạm vi phải nói rõ trong Limitations:** chỉ `wt_flair` (`src/baselines/unet2d.py` hardwire FLAIR+WT); **1 seed** (5 fold × 1 seed phủ trọn 368 BN — đủ cho phân rã, nhưng **không** có dải phương sai đa-seed); U-Net là **class B (học)** nên chỉ chấm out-of-fold, còn oracle là **class C (dùng GT lúc test)** — hai lớp khác nhau, so sánh này là **phân rã thông tin**, KHÔNG phải "U-Net thắng baseline".

---
## P3 (Goodhart) — DIỄN GIẢI CHỐT (2026-07-22, n=368)

**Provenance:** `results/stats/p3_delta.csv` ← `scripts/run_stats.py` (rule primary `brightest`) · `results/stats/{p3_by_rule,p3_spearman_by_rule,p3_psnr_monotone,p3_empty_mask_by_k}.csv` ← `scripts/analyse_p3.py --config configs/exp_main.yaml` (tái lập prereg §6/B4 ở n=368; đơn vị = bệnh nhân; `method=DP-exact` để **loại hoàn toàn nhiễu optimizer** — mọi khác biệt còn lại KHÔNG thể đổ cho metaheuristic).

### ✅ Phần SỐNG SÓT và BỀN (lõi của P3)

- **`mode_k_psnr = 10` ở 16/16 cấu hình** (2 target × 2 bg × 4 decoding rule). PSNR **luôn** chọn k lớn nhất.
- **Có chứng cứ giải tích + thực nghiệm:** PSNR là hàm đơn điệu của sai số lượng tử hoá (Lloyd–Max) ⇒ tăng theo k **BẰNG CẤU TRÚC**. Đo được: PSNR tăng **đơn điệu chặt** trên **cả 4** tổ hợp target×bg (24,9 → 36,9 dB). Đây là lập luận **không thể bị đánh bằng cỡ mẫu** (prereg §6/A4a: *"chứng minh, đừng tương quan"*).
- **`mode_k_dice` = 2, 3 hoặc 5 — KHÔNG BAO GIỜ = 10** (trừ ô suy biến `et_t1ce/brightest`).
- ⇒ **Sự bất đồng argmax là PHỔ QUÁT và CÓ CẤU TRÚC.** Đây là phát biểu chắc nhất của P3 và nên là câu headline.

### ⚠️ Phần THAY ĐỔI THEO DECODING RULE (phải báo cáo, không được giấu)

**Chi phí lâm sàng** của việc chọn k bằng PSNR — Δ median (`p3_by_rule.csv`, SESOI 0,01):

| target / bg | morph | brightest *(primary)* | otsu_pick | upper_union |
|---|---|---|---|---|
| wt_flair / false | **0,3210** ✅ | **0,1260** ✅ | 0,0125 ✅ | 0,0107 ❌ |
| wt_flair / true | **0,2707** ✅ | **0,1024** ✅ | 0,0081 ❌ | 0,0009 ❌ |
| et_t1ce / false | 0,0234 ✅ | 0,0000 ❌ | 0,0030 ❌ | 0,0012 ❌ |
| et_t1ce / true | 0,0314 ✅ | 0,0000 ❌ | 0,0006 ❌ | 0,0000 ❌ |

- **Thành công theo luật đã khoá (prereg A4a): 5/16 cấu hình.** Rule primary `brightest` **THÀNH CÔNG trên wt_flair cả hai bg** (0,126 / 0,102 — đây là con số vào Bảng), **THẤT BẠI trên et_t1ce** (Δ=0, `mode_k` trùng nhau ở 10).
- Biên độ chi phí trải từ **0,0009 đến 0,3210 Dice** — chênh **hơn 350 lần** — mà **thứ duy nhất thay đổi là quy tắc decoding**, không phải optimizer (DP-exact suốt), không phải hàm mục tiêu, không phải k.

### ❌ Phần THẤT BẠI — ghi nhận trung thực, KHÔNG cứu

- **P3-secondary (`ρ < −0,5`) THẤT BẠI ở 16/16 cấu hình.** Dấu **LẬT theo decoding rule** — tái lập đúng cảnh báo B4 ở n=368: `wt_flair/bg=false` → morph **−0,937** · brightest −0,226 (**p=0,156, không có ý nghĩa**) · otsu_pick −0,027 · upper_union **+0,288**. `et_t1ce/brightest` = **+0,991** — **ngược hẳn** dự đoán.
- ⇒ **Phát biểu gốc §1/P3 (*"k tăng thì Dice giảm"*) KHÔNG được ủng hộ ở dạng tổng quát và PHẢI RÚT.** Mọi câu dạng *"metrics are anti-correlated with clinical quality"* là **SAI** — cấm viết.
- **Mask rỗng theo k: ≈ 0 khắp nơi** (cao nhất 0,1% ở `et_t1ce/morph`, k=10). ⇒ Kỳ vọng của prereg dòng 386 (*"empty-mask rate là bằng chứng đẹp nhất cho P3"*) **KHÔNG thành hiện thực**. Báo cáo âm, không viết claim đó.
- **Trung thực bắt buộc (prereg §1/P3) — đã kiểm ở n=368:** *trong cùng một k*, tương quan PSNR–Dice **giữa các thuật toán** phần lớn **DƯƠNG** (et_t1ce: +0,891 ở k=3,4; wt_flair: +0,794 ở k=5; nhiễu hơn lô cũ, và **âm −0,855 ở k=2 cả hai target**). ⇒ **Nghịch lý nằm ở trục k, KHÔNG ở trục thuật toán.** Bài **phải nói rõ** điều này.

### 🎯 P3 VIẾT LẠI (bản dùng cho bản thảo)

> Thước đo mà dòng văn liệu dùng để chọn số ngưỡng **chọn k lớn nhất bằng cấu trúc toán học**, trong khi k tối ưu lâm sàng là **2–3**. Bất đồng này **phổ quát (16/16)**. Nhưng chi phí lâm sàng của nó **không do optimizer và không do metric quyết định** — nó bị chi phối bởi một **lựa chọn decoding hậu kiểm tuỳ tiện, gần như không bài nào báo cáo**, trải từ 0,001 đến 0,321 Dice.

**Vì sao bản này MẠNH HƠN bản gốc:** nó **hội tụ vào LUẬN ĐỀ DUY NHẤT** (decoding chi phối, optimizer là đồ trang trí) thay vì là một mệnh đề rời; và nó **sống sót** đúng phép kiểm mà prereg §6/B4 dựng ra để giết bản gốc. Vai trò của P3 đúng như prereg dòng 206 đã hạ cấp: **corroboration**, không phải trụ chính.
