...
The line of work treats multilevel thresholding as an optimization contest. A
criterion (most often Kapur's entropy or Otsu's between-class variance) is
fixed, and successive papers propose population-based search procedures to
maximize it, reporting the achieved criterion value, a reconstruction-fidelity
surrogate such as PSNR or SSIM, and convergence behavior. Quantum-inspired
variants are a visible branch of this activity, and the line remains active on
medical images into recent years~\cite{alnajdawi_scirep,mousavirad_kbs}. Two
features of the common evaluation practice matter for what follows. First,
performance is frequently reported in the criterion's own units or by a
fidelity surrogate, rather than against a segmentation ground truth. Second, the
map from the reported threshold vector to a scored binary mask is typically not
stated, so the object actually being measured is left implicit. We characterize
these as properties of the prevailing practice, not of any individual study, and
we attach no count to them in the absence of the coded survey.
...
Al-Najdawi \emph{et al.}~\cite{alnajdawi_scirep} provide a recent benchmark that
integrates a set of well-known optimization algorithms with Otsu's method for
multilevel thresholding of medical images, on the TCIA COVID-19-AR collection,
evaluating each optimizer by its reduction in computational cost while
maintaining segmentation quality. We treat this not as a competitor but as
evidence for the question we ask: the field continues to rank optimizers on
medical images by the criterion's own quality and cost measures. The precise
overlap figures and any ground-truth provisions of that study are not restated
here, as its full text was not accessible for verification at the time of
writing; we cite it for its setup, not for a number.
...
The gap these works leave is not an unsolved optimization but an unmeasured
projection: with an exact optimum on one side and a published ceiling on the
other, what has not been measured is whether the optimization distance separating
the competing methods survives the mapping onto a clinically scored mask. That
projection, on a cohort with ground truth, is the subject of this paper.
